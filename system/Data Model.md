...
**Beta 1** — single-user only. Multi-user/team collaboration (owner/assignee tagging, shared-vault conflict handling) is planned for a future version; see Feature requests below.
...
- **Body:** a `📦 Archive project` button, a `## Why` section (value chips, see below), freeform description/notes, a `## Note` section for linked Project Notes, and a `## Tasks` section containing the project's checkbox tasks.
